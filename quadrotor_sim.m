function quadrotor_sim
    ...
    fake_mass = 1;         %set uav mass [kg]
	fake_J = [0.01466 0 0;  %inertia matrix of uav
	          0 0.01466 0;
		      0 0 0.02848];
	...
	ideal_f_arr = zeros(1, ITERATION_TIMES);
	ideal_M_arr = zeros(3, ITERATION_TIMES);
	real_f_arr = zeros(1, ITERATION_TIMES);
	real_M_arr = zeros(3, ITERATION_TIMES);
	...
	for i = 1: ITERATION_TIMES
		...
		f_n = [0; 0; 0];
		f_n(1) = -(-kx(1)*ex(1) - kv(1)*ev(1) - fake_mass*uav_dynamics.g*e3(1) + fake_mass*a_d(1));
		f_n(2) = -(-kx(2)*ex(2) - kv(2)*ev(2) - fake_mass*uav_dynamics.g*e3(2) + fake_mass*a_d(2));
		f_n(3) = -(-kx(3)*ex(3) - kv(3)*ev(3) - fake_mass*uav_dynamics.g*e3(3) + fake_mass*a_d(3));
		...
		WJW = cross(uav_dynamics.W, fake_J * uav_dynamics.W);
		M_feedfoward = WJW - fake_J*(math.hat_map_3x3(uav_dynamics.W)*Rt*Rd*Wd - Rt*Rd*W_dot_d);
		uav_ctrl_M = [-kR(1)*eR(1) - kW(1)*eW(1) + M_feedfoward(1);
			      -kR(2)*eR(2) - kW(2)*eW(2) + M_feedfoward(2);
			      -kR(3)*eR(3) - kW(3)*eW(3) + M_feedfoward(3)];
        ideal_M = uav_ctrl_M;
        
        % Low-pass filter + motor efficiency modeling
        alpha_f = 1;
        alpha_M = 1;
        f_efficiency = 1;
        M_efficiency = 1;
        noise_f = 0 * f_total * randn(size(f_total));
        noise_M = 0 * norm(uav_ctrl_M) * randn(size(f_total));
        decayed_f = f_efficiency * f_total + noise_f;
        decayed_M = M_efficiency * uav_ctrl_M + noise_M;
        filtered_f = ((1 - alpha_f) * old_f) + (alpha_f * decayed_f);
        filtered_M = ((1- alpha_M) * old_M) + (alpha_M * decayed_M);
        old_f = filtered_f;
        old_M = filtered_M;
        ...
        A_inv = [1, -1/d, 1/d, -1/c;
                 ...
                 1, 1/d, -1/d, -1/c;
                 1, -1/d, -1/d, 1/c] / 4;
        f_motors_ideal = A_inv * [filtered_f; filtered_M(1); filtered_M(2); filtered_M(3)];
        ...
        real_f_arr(i) = filtered_f;
        real_M_arr(:, i) = filtered_M;
        ideal_f_arr(i) = f_total;
        ideal_M_arr(:, i) = ideal_M;
		ex_arr(:, i) = ex;
		ev_arr(:, i) = ev;
    end
	...
	save('sim_log.mat', 'dt', 'g', 'm', 'J', 'time_arr', ...
         'pos_arr', 'vel_arr', 'W_arr', 'R_arr', 'euler_arr', 'real_f_arr', 'real_M_arr', ...
         'ideal_f_arr', 'ideal_M_arr', 'f_motors_arr');
